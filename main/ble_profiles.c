...
void gatts_profiles_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
    ...
    case ESP_GATTC_CONNECT_EVT:{
        ...
        memcpy(gatts_profile_tab[DEVICE_PROFILE].remote_bda, p_data->connect.remote_bda, sizeof(esp_bd_addr_t));
        gatts_profile_tab[DEVICE_PROFILE].conn_id = p_data->connect.conn_id;
        
        ESP_LOGI(TAG_BLE, "REMOTE BDA:");
        esp_log_buffer_hex(TAG_BLE, gatts_profile_tab[DEVICE_PROFILE].remote_bda, sizeof(esp_bd_addr_t));
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == REMOTE_SERVICE_USERNAME_UUID) {
            ...
            gatts_profile_tab[DEVICE_PROFILE].service_start_handle = p_data->search_res.start_handle;
            gatts_profile_tab[DEVICE_PROFILE].service_end_handle = p_data->search_res.end_handle;
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if (server_attached){
            ...
            esp_gatt_status_t status = esp_ble_gattc_get_attr_count( gattc_if,
                                                                     ...
                                                                     gatts_profile_tab[DEVICE_PROFILE].service_start_handle,
                                                                     gatts_profile_tab[DEVICE_PROFILE].service_end_handle,
                                                                     INVALID_HANDLE,
                                                                     &count);
            ...
            if (count > 0){
                ...
                if (!char_elem_result){
                    ...
                    if (count > 0 && (char_elem_result[0].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)){
                        gatts_profile_tab[DEVICE_PROFILE].char_handle = char_elem_result[0].char_handle;
                        esp_ble_gattc_register_for_notify (gattc_if, gatts_profile_tab[DEVICE_PROFILE].remote_bda, char_elem_result[0].char_handle);
                    }
                }
                /* free char_elem_result */
                free(char_elem_result);
            ...
            if (adv_name != NULL) {
                if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ...
                    if (is_connected == false) {
                        ...
                        esp_ble_gattc_open(gatts_profile_tab[DEVICE_PROFILE].gattc_if, scan_result->scan_rst.bda, scan_result->scan_rst.ble_addr_type, true);
                    }
                }
            }
            break;
    ...
    do {
        ...
        for (index_profile = 0; index_profile < DEVICE_PROFILES; index_profile++) {
            if (gattc_if == ESP_GATT_IF_NONE || /* ESP_GATT_IF_NONE, not specify a certain gatt_if, need to call every profile cb function */
                    gattc_if == gatts_profile_tab[index_profile].gattc_if) {
                if (gatts_profile_tab[index_profile].gattc_cb) {
                    gatts_profile_tab[index_profile].gattc_cb(event, gattc_if, param);
                }
            }
        }
    } while (0);
